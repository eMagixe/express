...
import { b as _sfc_main$5$1, u as useComponentProps, t as tv, P as Primitive, g as getSlotChildrenText, c as _sfc_main$1$1, _ as _sfc_main$8, a as useForwardProps, h as useFormField, i as useComponentIcons, e as _sfc_main$3$1, l as looseToNumber } from './Button-Cn6n7Ulk.mjs';
import { _ as _sfc_main$9 } from './Modal-DCnjX90t.mjs';
import { a as useFetch } from './fetch-BtFdErkr.mjs';
import { _ as _sfc_main$5 } from './Container-D6wH7HZZ.mjs';
import { t as title_default } from './title-pte2Tibk.mjs';
import { _ as _sfc_main$a } from './Input-CIZ2z4xB.mjs';
...
import { _ as _sfc_main$7 } from './Drawer-Bp0mVmZt.mjs';
import { u as usePrefix } from './usePrefix-wIR1GMpN.mjs';
import { _ as _sfc_main$6 } from './PricingPlan-DlQaA4Wu.mjs';
			...
			_push(ssrRenderComponent(_component_UContainer, { class: "h-[calc(100vh-100px)] min-h-160 flex flex-col justify-start gap-5 pt-10" }, {
				default: withCtx((_, _push, _parent, _scopeId) => {
					if (_push) {
						...
						createVNode("div", { class: "cities-list w-full flex flex-row justify-center items-center gap-10" }, [(openBlock(true), createBlock(Fragment, null, renderList(unref(directions), (direction) => {
						...
						createVNode("div", { class: "flex flex-col md:flex-row justify-between items-center gap-2 py-2 px-3 rounded-lg bg-gradient-to-r from-gray-600/30 to-gray-700/30 backdrop-blur-[5px] mx-5" }, [createVNode("p", { class: "text-sm text-white" }, "* требуется дополнительная оплата"), createVNode(_component_NuxtLink, {
							class: "text-primary underline flex flex-row justify-center items-center gap-2",
							to: "/directions"
						}, {
...
var virtual_nuxt_node_modules_2F_cache_2Fnuxt_2F_nuxt_2Fui_2Fcarousel_default = {
	...
	"variants": {
		...
		"active": { "true": { "dot": "data-[state=active]:bg-inverted" } }
	}
};
//#endregion
//#region node_modules/@nuxt/ui/dist/runtime/components/Carousel.vue
var _sfc_main$4 = {
	__name: "UCarousel",
	__ssrInlineRender: true,
	props: {
	...
	setup(__props, { expose: __expose, emit: __emit }) {
		const props = useComponentProps("carousel", __props);
		...
		const stopAutoplayOnInteraction = computed(() => {
			if (typeof props.autoplay === "boolean") return true;
			return props.autoplay?.stopOnInteraction ?? true;
		});
		const stopAutoScrollOnInteraction = computed(() => {
			if (typeof props.autoScroll === "boolean") return true;
			return props.autoScroll?.stopOnInteraction ?? true;
		});
		const ui = computed(() => tv({
			extend: virtual_nuxt_node_modules_2F_cache_2Fnuxt_2F_nuxt_2Fui_2Fcarousel_default,
			...appConfig.ui?.carousel || {}
		...
		watch(() => [
			...
			props.fade,
			props.wheelGestures
		], loadPlugins, { immediate: true });
		const [emblaRef, emblaApi] = useEmblaCarousel(options, plugins);
		watch(options, () => {
			emblaApi.value?.reInit(options.value, plugins.value);
		}, { flush: "post" });
		function stopOnInteraction() {
			if (stopAutoplayOnInteraction.value) emblaApi.value?.plugins().autoplay?.stop();
			if (stopAutoScrollOnInteraction.value) emblaApi.value?.plugins().autoScroll?.stop();
		}
		function scrollPrev() {
		...
		return (_ctx, _push, _parent, _attrs) => {
			_push(ssrRenderComponent(unref(Primitive), mergeProps({
				...
				default: withCtx((_, _push, _parent, _scopeId) => {
					if (_push) {
						_push(`<div data-slot="viewport" class="${ssrRenderClass(ui.value.viewport({ class: unref(props).ui?.viewport }))}"${_scopeId}><div data-slot="container" class="${ssrRenderClass(ui.value.container({ class: unref(props).ui?.container }))}"${_scopeId}><!--[-->`);
						ssrRenderList(unref(props).items, (item, index) => {
							_push(`<div${ssrRenderAttrs(mergeProps({ key: index }, { ref_for: true }, unref(props).dots ? { role: "tabpanel" } : {
								"role": "group",
								...
								class: ui.value.item({ class: [
									unref(props).ui?.item,
									isCarouselItem(item) && item.ui?.item,
						...
						if (unref(props).arrows || unref(props).dots) {
							...
							if (unref(props).dots) {
								_push(`<div role="tablist"${ssrRenderAttr("aria-label", unref(t)("carousel.dots"))} data-slot="dots" class="${ssrRenderClass(ui.value.dots({ class: unref(props).ui?.dots }))}"${_scopeId}><!--[-->`);
								ssrRenderList(scrollSnaps.value, (_, index) => {
									_push(`<button type="button" role="tab"${ssrRenderAttr("aria-label", unref(t)("carousel.goto", { slide: index + 1 }))}${ssrRenderAttr("aria-selected", selectedIndex.value === index)} data-slot="dot" class="${ssrRenderClass(ui.value.dot({
										class: unref(props).ui?.dot,
										active: selectedIndex.value === index
									}))}"${ssrRenderAttr("data-state", selectedIndex.value === index ? "active" : void 0)}${_scopeId}></button>`);
								});
								_push(`<!--]--></div>`);
							} else _push(`<!---->`);
							_push(`</div>`);
						...
						return openBlock(), createBlock("div", mergeProps({ key: index }, { ref_for: true }, unref(props).dots ? { role: "tabpanel" } : {
							"role": "group",
							"aria-roledescription": "slide"
						}, {
							"data-slot": "item",
					...
					}), 128))], 2)], 2), unref(props).arrows || unref(props).dots ? (openBlock(), createBlock("div", {
						key: 0,
						"data-slot": "controls",
						...
						"aria-label",
						"class"
					])], 2)) : createCommentVNode("", true), unref(props).dots ? (openBlock(), createBlock("div", {
						key: 1,
						role: "tablist",
						"aria-label": unref(t)("carousel.dots"),
						"data-slot": "dots",
						class: ui.value.dots({ class: unref(props).ui?.dots })
					}, [(openBlock(true), createBlock(Fragment, null, renderList(scrollSnaps.value, (_, index) => {
					...
					}), 128))], 10, ["aria-label"])) : createCommentVNode("", true)], 2)) : createCommentVNode("", true)];
				}),
				_: 3
			}, _parent));
		};
			...
			_push(ssrRenderComponent(_component_UDrawer, mergeProps({
				...
				content: withCtx((_, _push, _parent, _scopeId) => {
					if (_push) {
							...
							default: withCtx(({ item }, _push, _parent, _scopeId) => {
								if (_push) {
									...
									return openBlock(), createBlock("img", {
										height: "20",
										width: "20",
										src: _virtual_public__2Fimages_2Fsvg_2Fstar_default,
										alt: `star +${n}`
									}, null, 8, ["alt"]);
								}), 256))])]), createVNode("div", { class: "flex flex-row justify-center items-center gap-1 text-gray-600" }, [createVNode(_component_UIcon, {
									name: "i-lucide-calendar",
									size: "14"
								}), createVNode("p", { class: "text-[clamp(.8rem,1vw,1rem)]" }, toDisplayString(item.date), 1)])]), createVNode("div", { class: "text-gray-600 text-[1rem]" }, toDisplayString(item.text), 1)])];
							}),
							_: 1
						}, _parent, _scopeId));
						_push(`</div>`);
					} else return [createVNode("div", { class: "modal-reviews" }, [createVNode(_component_UCarousel, {
						loop: "",
						autoplay: { delay: 2200 },
						items: unref(reviews),
						ui: { item: "not-lg:basis-1/1 md:basis-1/2 lg:basis-1/3" },
						...
						default: withCtx(({ item }) => [createVNode("div", { class: "item flex flex-col justify-start items-start gap-1" }, [createVNode("div", { class: "flex w-full flex-row justify-between items-start gap-2" }, [createVNode("div", { class: "flex xl:flex-row flex-col lg:justify-center lg:items-center gap-5 text-gray-600" }, [createVNode("div", { class: "uppercase text-[14px] not-lg:text-[30px]" }, [createVNode(_component_UIcon, { name: "i-lucide-circle-user-round" }), createTextVNode(" " + toDisplayString(item.name), 1)]), createVNode("div", { class: "flex flex-row justify-start items-start gap-1 mb-5" }, [(openBlock(true), createBlock(Fragment, null, renderList(item.rating, (n) => {
							return openBlock(), createBlock("img", {
								height: "20",
								width: "20",
								src: _virtual_public__2Fimages_2Fsvg_2Fstar_default,
...
//#endregion
//#region app/components/preview/all.vue
var _sfc_setup$7 = all_vue_vue_type_script_setup_true_lang_default.setup;
all_vue_vue_type_script_setup_true_lang_default.setup = (props, ctx) => {
	const ssrContext = useSSRContext();
...
var _sfc_setup$6 = _sfc_main$3.setup;
...
//#endregion
//#region app/components/preview/add.vue?vue&type=script&setup=true&lang.ts
var add_vue_vue_type_script_setup_true_lang_default = /*@__PURE__*/ defineComponent({
	__name: "add",
	__ssrInlineRender: true,
	setup(__props) {
			...
			_push(ssrRenderComponent(_component_UModal, mergeProps({
				...
				body: withCtx((_, _push, _parent, _scopeId) => {
					if (_push) {
						...
						_push(`<div class="flex flex-row justify-start items-start gap-1 pl-1 w-full cursor-pointer"${_scopeId}><!--[-->`);
						ssrRenderList(5, (n) => {
							_push(`<img height="20" width="20"${ssrRenderAttr("src", _virtual_public__2Fimages_2Fsvg_2Fstar_default)} class="${ssrRenderClass({ grayscale: n > unref(current).rating })}"${ssrRenderAttr("alt", `star +${n}`)}${_scopeId}>`);
						});
						_push(`<!--]--></div>`);
						...
						_push(`<div class="w-full flex justify-center items-center"${_scopeId}>`);
						...
						createVNode("div", { class: "flex flex-row justify-start items-start gap-1 pl-1 w-full cursor-pointer" }, [(openBlock(), createBlock(Fragment, null, renderList(5, (n) => {
							return createVNode("img", {
								height: "20",
								width: "20",
								src: _virtual_public__2Fimages_2Fsvg_2Fstar_default,
								class: { grayscale: n > unref(current).rating },
						...
						createVNode("div", { class: "w-full flex justify-center items-center" }, [createVNode(_component_UButton, {
							onClick: unref(add),
							class: "button-gradient",
							icon: "i-lucide-plus"
						}, {
...
//#endregion
//#region app/components/preview/add.vue
var _sfc_setup$5 = add_vue_vue_type_script_setup_true_lang_default.setup;
add_vue_vue_type_script_setup_true_lang_default.setup = (props, ctx) => {
	const ssrContext = useSSRContext();
	(ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("components/preview/add.vue");
	return _sfc_setup$5 ? _sfc_setup$5(props, ctx) : void 0;
};
var add_default = Object.assign(add_vue_vue_type_script_setup_true_lang_default, { __name: "PreviewAdd" });
//#endregion
...
function _sfc_ssrRender(_ctx, _push, _parent, _attrs) {
	...
	const _component_PreviewAdd = add_default;
	...
	_push(ssrRenderComponent(_component_UContainer, { class: "flex flex-col justify-center items-center gap-5" }, {
		default: withCtx((_, _push, _parent, _scopeId) => {
			if (_push) {
				_push(ssrRenderComponent(_component_SectionTitle, { title: "Отзывы" }, null, _parent, _scopeId));
				_push(`<div class="w-full flex flex-col justify-start items-center pt-10 gap-10" data-v-a563bc95${_scopeId}>`);
				_push(ssrRenderComponent(_component_PreviewViewer, null, null, _parent, _scopeId));
				_push(`<div class="w-full flex flex-row justify-between items-center pt-10 gap-5" data-v-a563bc95${_scopeId}>`);
				_push(ssrRenderComponent(_component_PreviewAll, null, null, _parent, _scopeId));
				_push(ssrRenderComponent(_component_PreviewAdd, null, null, _parent, _scopeId));
				_push(`</div></div>`);
			} else return [createVNode(_component_SectionTitle, { title: "Отзывы" }), createVNode("div", { class: "w-full flex flex-col justify-start items-center pt-10 gap-10" }, [createVNode(_component_PreviewViewer), createVNode("div", { class: "w-full flex flex-row justify-between items-center pt-10 gap-5" }, [createVNode(_component_PreviewAll), createVNode(_component_PreviewAdd)])])];
		}),
		_: 1
	}, _parent));
	_push(`</div>`);
...
var reviews_default = /*#__PURE__*/ Object.assign(_plugin_vue_export_helper_default(_sfc_main$2, [["ssrRender", _sfc_ssrRender], ["__scopeId", "data-v-a563bc95"]]), { __name: "SectionReviews" });
//#endregion
//#region virtual:nuxt:node_modules%2F.cache%2Fnuxt%2F.nuxt%2Fui%2Fpage-list.ts
var virtual_nuxt_node_modules_2F_cache_2Fnuxt_2F_nuxt_2Fui_2Fpage_list_default = {
	"base": "relative flex flex-col",
	"variants": { "divide": { "true": "*:not-last:after:absolute *:not-last:after:inset-x-1 *:not-last:after:bottom-0 *:not-last:after:bg-border *:not-last:after:h-px" } }
};
//#endregion
//#region node_modules/@nuxt/ui/dist/runtime/components/PageList.vue
var _sfc_main$1 = {
	__name: "UPageList",
	__ssrInlineRender: true,
	props: {
	...
	setup(__props) {
		const props = useComponentProps("pageList", __props);
		const appConfig = useAppConfig();
		const ui = computed(() => tv({
			extend: virtual_nuxt_node_modules_2F_cache_2Fnuxt_2F_nuxt_2Fui_2Fpage_list_default,
			...appConfig.ui?.pageList || {}
		}));
		return (_ctx, _push, _parent, _attrs) => {
			_push(ssrRenderComponent(unref(Primitive), mergeProps({
				as: unref(props).as,
...
//#endregion
//#region node_modules/@nuxt/ui/dist/runtime/components/PageCard.vue
					...
					if (_push) {
						...
						if (!!slots.header || unref(props).icon || !!slots.leading || !!slots.body || unref(props).title || !!slots.title || unref(props).description || !!slots.description || !!slots.footer) {
							...
							if (unref(props).icon || !!slots.leading) {
								...
								ssrRenderSlot(_ctx.$slots, "leading", { ui: ui.value }, () => {
									if (unref(props).icon) _push(ssrRenderComponent(_sfc_main$5$1, {
						...
						createVNode("div", {
							...
							unref(props).icon || !!slots.leading ? (openBlock(), createBlock("div", {
								...
								"data-slot": "leading",
								class: ui.value.leading({ class: unref(props).ui?.leading })
							}, [renderSlot(_ctx.$slots, "leading", { ui: ui.value }, () => [unref(props).icon ? (openBlock(), createBlock(_sfc_main$5$1, {
								key: 0,
								name: unref(props).icon,
								"data-slot": "leadingIcon",
								class: ui.value.leadingIcon({ class: unref(props).ui?.leadingIcon })
...
//#endregion
//#region app/components/section/questions.vue?vue&type=script&setup=true&lang.ts
	...
	setup(__props) {
		...
		return (_ctx, _push, _parent, _attrs) => {
			...
			_push(ssrRenderComponent(_component_UContainer, { class: "flex flex-col justify-center items-center gap-5" }, {
				default: withCtx((_, _push, _parent, _scopeId) => {
					if (_push) {
						_push(ssrRenderComponent(_component_SectionTitle, { title: "Популярные вопросы" }, null, _parent, _scopeId));
						_push(`<div class="w-full animation-box flex flex-col justify-start items-center pt-10 gap-10" data-v-28f06034${_scopeId}>`);
						_push(ssrRenderComponent(_component_UPageList, { class: "questions" }, {
							default: withCtx((_, _push, _parent, _scopeId) => {
								if (_push) {
									_push(`<!--[-->`);
									ssrRenderList(items, (item, index) => {
										_push(ssrRenderComponent(_component_UPageCard, {
											...
											variant: "ghost",
											class: ["rounded-none", { "border-b border-b-gray-200": index !== items.length - 1 }]
										}, {
											body: withCtx((_, _push, _parent, _scopeId) => {
												if (_push) _push(`<h3 class="text-xl mb-5" data-v-28f06034${_scopeId}>${ssrInterpolate(item.label)}</h3><p class="text-sm" data-v-28f06034${_scopeId}>${ssrInterpolate(item.content)}</p>`);
												else return [createVNode("h3", { class: "text-xl mb-5" }, toDisplayString(item.label), 1), createVNode("p", { class: "text-sm" }, toDisplayString(item.content), 1)];
											}),
											_: 2
										}, _parent, _scopeId));
									});
									_push(`<!--]-->`);
								} else return [(openBlock(), createBlock(Fragment, null, renderList(items, (item, index) => {
									return createVNode(_component_UPageCard, {
										key: ("useId" in _ctx ? _ctx.useId : unref(useId))(),
										variant: "ghost",
										class: ["rounded-none", { "border-b border-b-gray-200": index !== items.length - 1 }]
									}, {
										body: withCtx(() => [createVNode("h3", { class: "text-xl mb-5" }, toDisplayString(item.label), 1), createVNode("p", { class: "text-sm" }, toDisplayString(item.content), 1)]),
										_: 2
									}, 1032, ["class"]);
								}), 64))];
							}),
						...
						_push(`</div>`);
					} else return [createVNode(_component_SectionTitle, { title: "Популярные вопросы" }), createVNode("div", { class: "w-full animation-box flex flex-col justify-start items-center pt-10 gap-10" }, [createVNode(_component_UPageList, { class: "questions" }, {
						default: withCtx(() => [(openBlock(), createBlock(Fragment, null, renderList(items, (item, index) => {
							return createVNode(_component_UPageCard, {
								key: ("useId" in _ctx ? _ctx.useId : unref(useId))(),
								variant: "ghost",
								class: ["rounded-none", { "border-b border-b-gray-200": index !== items.length - 1 }]
...
//#endregion
//#region app/components/section/questions.vue
var _sfc_setup$1 = questions_vue_vue_type_script_setup_true_lang_default.setup;
...
var __nuxt_component_4_lazy = defineAsyncComponent(() => import('./order-BuBdFEjc.mjs').then((c) => c.default || c));
var index_vue_vue_type_script_setup_true_lang_default = /*@__PURE__*/ defineComponent({
	...
	setup(__props) {
		...
		return (_ctx, _push, _parent, _attrs) => {
			const _component_SectionWelcome = welcome_default;
			const _component_SectionAdvantages = advantages_default;
			const _component_SectionReviews = reviews_default;
			const _component_SectionQuestions = questions_default;
